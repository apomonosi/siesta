...
{\Huge \textbf{T B t r a n s \, \, \softwareversion}\par}
...
\tbtrans\ will be compiled when together with \siesta\ when issuing
\shell{cmake}.

If only the \tbtrans/\phtrans\ executables are desired, say for tight-binding
calculations, one should execute the \shell{cmake} build like this:
\begin{shellexample}
...
\begin{description}
  \item[\fdf*{-help} \textnormal{or} \fdf*{-h}] print a help instruction and quit

  \item[\fdf*{-version} \textnormal{or} \fdf*{-v}] print \tbtrans\ version and quit

  \item[\fdf*{-out} \textnormal{or} \fdf*{-o}] specify where all output should be written to
  (instead of STDOUT)
  ...
  $0.2\,\mathrm{eV}$. A value without unit is interpreted as
  $\mathrm{eV}$.
...
\begin{fdfentry}{TBT.HS}[file]<\fdf*{<SystemLabel>}.TS.HSX>
...
\begin{fdfexample}
  %block TBT.HS.Files
    ../V0/siesta.TS.HSX     0.  eV
    ../V-0.5/siesta.TS.HSX -0.5 eV
    ../V0.5/siesta.TS.HSX   0.5 eV
    ../V-1.0/siesta.TS.HSX -1.0 eV
    ../V1.0/siesta.TS.HSX   1.0 eV
  %endblock
\end{fdfexample}
...
as quantities will be orbital resolved.
...
\begin{fdflogicalF}{TBT.DOS!Gf}
  ...
  \begin{align}
    \label{eq:dos-gf}
    \mathbf G(E) &= [E\mathbf S - \mathbf H -
    \sum_{\mathfrak e}\boldsymbol\Sigma_{\mathfrak e}(E)]^{-1}
    \\
    &=
    \sum_{\mathfrak e}\mathbf G(E)\boldsymbol\Gamma_{\mathfrak
        e}(E)\mathbf G^\dagger(E) + \text{bound states}
    \\
    &=
    \sum_{\mathfrak e}\mathbf A_{\mathfrak e}(E) + \text{bound states}
  \end{align}
...
\begin{fdflogicalF}{TBT.DOS!A}
  ...
  Its relation to the Green function DOS can be inferred from
  Eq.~\eqref{eq:dos-gf} (see \fdf{TBT.DOS!Gf}). If there are no bound
  states in the device region then prefer this option and
  \fdf{TBT.DOS!A.All}.
...
\begin{fdfentry}{TBT.k}[list/block]<\nonvalue{kgrid\_Monkhorst\_Pack}>
  \fdfdepend{TBT.k.File}

  If \fdf{TBT.k.File} is present, that will have precedence.
  ...
  path 20
    ...
    \begin{fdfoptions}
      ...
      required points required on each path to have the same $\delta k$
      in each path.
...
\begin{fdfentry}{TBT.k.File}[string]<NONE>

  If provided, it is a (relative) path to a file containing
  $k$-points in the filename containing points in units of the
  reciprocal lattice vectors. I.e. in the range of $]-1/2 ; 1/2]$.
  See e.g. \fdf{TBT.k!list} for details, or see \fdf*{kgrid.File} in the \siesta\ manual
  for more details about the exact file layout.

  The only exception is that the weights need not sum to $1$ as is the case for \siesta.
  This is because \tbtrans\ can do reduced Brillouin zone calculations. Hence,
  be careful about what you pass.
...
\begin{fdfentry}{TBT.Contours!Eta}[energy]<$\operatorname{min}[\eta_{\mathfrak e}]/10$>
  ...
  axis. Generally one need not take into account $\eta$ values
  different from 0. However, in cases where localized states are found
  a smearing in the device region can help numerics. Therefore it
  defaults to $\operatorname{min}[\eta_{\mathfrak e}]/10$. This ensures
  that the device broadening is always smaller than the electrodes
  while allowing broadening of localized states.
...
\begin{fdfentry}{TBT.Elec.<>}[block]
  ...
  \begin{fdfoptions}
    ...
    for this.
    The NetCDF4 files can also be created using \sisl.
    ...
    \fdfdepend{TBT.Elec.<>!semi-inf-direction}%
    ...
    then this can greatly improve throughput. It has a cost of disk-space.
    ...
    electrode/device interface). It \emph{should not} be used for converged
    \tsiesta\ calculations.
...
\begin{fdfentry}{TBT.Projs!T}[block]%
  \fdfdepend{TBT.Projs,TBT.Projs!T.All,TBT.Projs!T.out}

  As one might specify \emph{many} molecular projections to
...
By default some electrodes are not accessible for projections unless
\fdf{TBT.Projs!T.Out} or \fdf{TBT.Projs!T.All} are \fdftrue.
