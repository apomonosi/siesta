...
# define GEMM zgemm
  ...
  complex(dp), parameter, dimension(2,2) :: I2 = reshape((/1._dp, 0._dp, 0._dp, 1._dp/), (/2,2/))
  
contains

  subroutine tbt_trik_spinor(N_Elec, Elecs, TSHS, nq, uGf)
    ...
    ! Gf.Gamma.Gf from ts_tri_scat_spinor and all remaining routines
    use m_tbt_tri_scat_spinor
...
! ******************* Computational arrays *******************
    ...
    logical :: calc_T_Spin, calc_T_Spin_Fast
#ifdef NCDF_4
...
! ********************** Result arrays ***********************
    ...
    real(dp) :: Tspin(2,2,N_Elec+1,N_Elec)
    ...
    type(dSpData2D) :: dev_DM
    real(dp), pointer :: val_dev_M(:), val_dev_DM(:,:)
#endif
! ************************************************************
...
#endif
    integer :: n_kpt
    complex(dp) :: sproj(2,2)
    complex(dp), allocatable, target :: sqrtGamma(:,:), projGamma(:)
    complex(dp), allocatable, target :: zproj_work(:)
    complex(dp), pointer :: old_Gamma(:)
...
! ************************************************************
    ...
    calc_T_Spin = 'T-Spin' .in. save_DATA
    calc_T_Spin_Fast = 'T-Spin-Fast' .in. save_DATA
    ...
    if ( calc_proj_T_out       .or. &
         calc_proj_DOS_A       .or. &
         calc_proj_orb_current .or. &
         calc_proj_DM_A        .or. &
         calc_proj_COOP_A      .or. &
         calc_proj_COHP_A )  then 
          
          call die('Projections not yet supported for spinor calculation')
    end if
    ...
    ! Since we need to expand each matrix element into 2x2 blocks we
    ! mulutply by 4 here. Otherwise, we could adjust the sizes at every
    ! step above, but this is more efficient and the result should be the same.
    pad_RHS = 4 * pad_RHS
    pad_LHS = 4 * pad_LHS
    nGFGGF = 4 * nGFGGF
    ...
    ! We multiply the size of each part (DevTri%r) by a factor of 2 to account
    ! for spin up and down
    call print_memory('LHS',pad_LHS)
    call newzTriMat(zwork_tri,DevTri%n,2*DevTri%r,'GFinv', &
         padding = pad_LHS )

    call print_memory('RHS',pad_RHS)
    call newzTriMat(GF_tri,DevTri%n,2*DevTri%r,'GF', &
        padding = pad_RHS )
    ...
    if ( N_eigen > 0 ) then
       ...
       tt_work => zwork(nzwork-4*info+1:nzwork-4*no)
       ntt_work = size(tt_work)
       eig => zwork(nzwork-4*no+1:nzwork)
    ...
    do iEl = 1 , N_Elec
       ...
       io = 4 * max(Elecs(iEl)%device_orbitals(),Elecs(iEl)%o_inD%n) ** 2
    ...
    if ( calc_T_Spin ) then
      io = 0
      do iEl = 1 , N_Elec
        io = max(io, Elecs(iEl)%o_inD%n)
      end do
      allocate(projGamma(4*io*io))
      if ( .not. calc_T_Spin_Fast ) then
        allocate(sqrtGamma(4*io*io,2))
        allocate(zproj_work(4*io*io))
      end if
    end if
    ...
    call newzSpData3D(sp_uc,2,2,fdist,spH,name='TBT spH',sparsity_dim=3)
    ...
    if ( .not. calc_DOS_A ) then
      ...
      do jEl = N_Elec2 , N_Elec
        ...
        A_parts(which_part(Gf_tri, 2*(iEl-1)+1)) = .true.
        A_parts(which_part(Gf_tri, 2*(iE-1)+1)) = .true.
      end do
    ...
    call BiasTrimat_prep(zwork_tri,N_Elec,Elecs,prep_El,part_cols,basis_size=2)
    deallocate(prep_El)
    ...
    if ( iEl == 1 ) then
      ...
      val_dev_M => val(dev_M)
      call newdSpData2D(sp_dev_sc,spin%DM,fdist,dev_DM,name='TBT sparse DM')
      val_dev_DM => val(dev_DM)
    end if
#endif
    ...
    iounits = -100
    call init_save(iounits,1,TSHS%nspin,r_oDev%n, N_Elec,Elecs, N_eigen, &
         save_DATA)
    allocate(iounits_El(N_Elec*2)) ! maximum number of units
    iounits_El = -100
    call init_save_Elec(iounits_El,1,TSHS%nspin,N_Elec,Elecs, &
         save_DATA)
#endif
    ...
    do while ( .not. itt_step(Kp) )
       ...
       iE = size(S)
       if ( n_k == 0 ) then
          call create_HS(TSHS%dit, sp, spin%H, 0._dp, &
       ...
       do while ( cE%exist ) 
...
#ifdef NCDF_4
             if ( calc_DM_Gf ) then
               call Gf_DM(TSHS%sc_off,kpt,phase,Gf_tri,zwork_tri,r_oDev,pvt, dev_DM)
#ifdef TBT_PHONON
               call dscal(size(val_dev_DM), 2._dp * omega, val_dev_DM(1), 1)
#endif
               call state_cdf_save_sp_dev(TBTcdf, ikpt, nE, 'DM', dev_DM)
             end if
             ...
             if ( calc_DOS_A ) then
...
#ifdef NCDF_4
                if ( calc_DM_A ) then
                  call A_DM(TSHS%sc_off,kpt,phase,zwork_tri,r_oDev,pvt, dev_DM)
#ifdef TBT_PHONON
                  call dscal(size(val_dev_DM), 2._dp * omega, val_dev_DM(1), 1)
#endif
                  call state_cdf_save_sp_dev(TBTcdf, ikpt, nE, 'DM', dev_DM, &
                ...
                if ( calc_orb_current ) then
                   ...
                   call orb_current(TSHS%sp,H,S,TSHS%sc_off, &
                        kpt, phase, &
                        cOmega,zwork_tri,r_oDev,dev_DM,pvt)
#else
                   call orb_current(TSHS%sp,H,S,TSHS%sc_off, &
                        kpt, phase, &
                        cE,zwork_tri,r_oDev,dev_DM,pvt)
#endif
                  !  if ( dH%lvl > 0 ) then
                  ...
                  !  end if
                   ...
                   call state_cdf_save_sp_dev(TBTcdf, ikpt, nE, 'J', dev_DM, &
                        Elecs(iEl))
          ...
          if ( calc_T_Spin ) then
            ...
            if ( calc_T_Gf ) then
              ...
              call die('TBT.T.Gf does not work with TBT.T.Spin for now')
              ...
              do iEl = 1 , N_Elec1
                ...
                ! Project Elec(iEl)%Gamma onto a specifc spin direction
                call calc_sqrt_Gamma(Elecs(iEl)%o_inD%n, Elecs(iEl)%Gamma, &
                    sqrtGamma(:,1), zproj_work)

              end if

              do ispin = 1, 2
                sproj(:,:) = spin_projector(Elecs(iEl)%spin_ax, &
                    anti_parallel=(ispin == 2))

                if ( calc_T_Spin_Fast ) then
                  call P_Gamma_P(Elecs(iEl)%Gamma, Elecs(iEl)%o_inD%n, &
                  sproj(:,:), sproj(:,:), projGamma)
                else
                  call project_Gamma(Elecs(iEl)%o_inD%n, sqrtGamma(:,1), &
                      sproj(:,:), projGamma, zproj_work)
                end if

                ! Point El%Gamma to projGamma
                old_Gamma => Elecs(iEl)%Gamma
                Elecs(iEl)%Gamma => projGamma
                  ...
                  if ( ts_A_method == TS_BTD_A_COLUMN ) then
                    ...
                    if ( calc_T_out ) then
                      call Gf_Gamma(zwork_tri,Elecs(iEl), &
                          Tspin(ispin,ispin,N_Elec+1,iEl))
                    end if
                    ...
                    if ( calc_T_out ) then
                      call dir_GF_Gamma_GF(Gf_tri, zwork_tri, r_oDev, pvt, &
                          Elecs(iEl), A_parts, &
                          TrGfG=Tspin(ispin,ispin,N_Elec+1,iEl))
                    else
                      call dir_GF_Gamma_GF(Gf_tri, zwork_tri, r_oDev, pvt, &
                          Elecs(iEl), A_parts)
                    end if
                ...
                ! Restore pointer to full Gamma
                Elecs(iEl)%Gamma => old_Gamma
                ...
                do jEl = N_Elec2 , N_Elec
                  ...
                  if ( .not. calc_T_Spin_Fast ) then
                    call calc_sqrt_Gamma(Elecs(jEl)%o_inD%n, Elecs(jEl)%Gamma, &
                    sqrtGamma(:,2), zproj_work)
                  end if
                  
                  do jspin = 1, 2
                    
                  sproj(:,:) = spin_projector(Elecs(jEl)%spin_ax, &
                      anti_parallel=(jspin == 2))

                  if ( calc_T_Spin_Fast ) then
                    call P_Gamma_P(Elecs(jEl)%Gamma, Elecs(jEl)%o_inD%n, &
                    sproj(:,:), sproj(:,:), projGamma)
                  else
                    call project_Gamma(Elecs(jEl)%o_inD%n, sqrtGamma(:,2), &
                        sproj(:,:), projGamma, zproj_work)
                  end if
  
                  ! Point El%Gamma to projGamma
                  old_Gamma => Elecs(jEl)%Gamma
                  Elecs(jEl)%Gamma => projGamma
                  ...
                  if ( .not. cE%fake ) then
                    call A_Gamma(zwork_tri,Elecs(jEl), &
                        Tspin(jspin,ispin,jEl,iEl))
                  end if

                  ! Restore pointer to full Gamma
                  Elecs(jEl)%Gamma => old_Gamma
                end do ! jspin
          ...
          call state_cdf_save(TBTcdf, ikpt, nE, N_Elec, Elecs, &
               DOS, T, N_eigen, Teig, Tspin, save_DATA)
#else
          call state_save(iounits,r_oDev%n, nE,N_Elec,Elecs,DOS, T, &
               N_eigen, Teig, Tspin, save_DATA )
#endif
#ifdef NCDF_4
...
!                  call A_DM(TSHS%sc_off,kpt,phase,zwork_tri,r_oDev,pvt, dev_DM)
! #ifdef TBT_PHONON
!                  call dscal(4*size(val_dev_DM), 2._dp * omega, val_dev_DM(1), 1)
! #endif
!                  call proj_cdf_save_sp2D_dev(PROJcdf, ikpt, nE, 'DM', p_E, dev_DM)
!                end if
!                if ( calc_proj_COOP_A ) then
!                  call A_COOP(r_oDev,zwork_tri,pvt, &
!                      TSHS%sp,S,TSHS%sc_off, kpt, phase, dev_M)
...
!             end if
    ...
    if ( calc_T_Spin ) then
      deallocate(projGamma)
      if ( .not. calc_T_Spin_Fast ) then
        deallocate(sqrtGamma)
        deallocate(zproj_work)
      end if
    end if
  ...
  end subroutine tbt_trik_spinor
  ...
  subroutine prepare_invGF(cE, GFinv_tri, r, pvt, &
    ...
    use m_tbt_tri_scat_spinor, only : insert_Self_Energy_Dev
#ifdef NCDF_4
    ...
    do iu = 1, r%n
       ...
       do ind = l_ptr(io) + 1 , l_ptr(io) + l_ncol(io)
          ...
          if ( ju > 0 ) then
            ...
            GFinv(index(Gfinv_tri,2*ju-1,2*iu-1)) = Z * S(ind) -  H(1,1,ind)
            GFinv(index(Gfinv_tri,2*ju-1,2*iu  )) =            -  H(1,2,ind)
            GFinv(index(Gfinv_tri,2*ju  ,2*iu-1)) =            -  H(2,1,ind)
            GFinv(index(Gfinv_tri,2*ju  ,2*iu  )) = Z * S(ind) -  H(2,2,ind)
...
!$OMP end parallel
  ...
  subroutine downfold_SE(cE, El, spH, spS, r, &
    ...
    type(zSpData3D), intent(in) :: spH
    type(zSpData1D), intent(in) :: spS
    ...
    integer :: sN, sNm1, sNSQ, sNm1SQ, sN_h, sNm1_h
    integer :: ip, itmp
    ...
    itmp = p(np-1) ** 2 ! A
    itmp = itmp + p(np) ** 2 ! Y
    if ( 4 * itmp > nwork ) then ! we do not need y here...
    ...
    do ip = 2 , np
      ...
      ! Half sizes (used only for indexing purposes)
      sNm1_h = p(ip-1)
      sN_h = p(ip)
      ...
      call prep_HS(cE%E,El,spH,spS,r,off,sNm1_h,off,sNm1_h,A, sc_off, kpt)
      ...
      i = off + sNm1_h
      call prep_HS(cE%E,El,spH,spS,r,i,sN_h,off,sNm1_h,B, sc_off, kpt)
      call prep_HS(cE%E,El,spH,spS,r,off,sNm1_h,i,sN_h,C, sc_off, kpt)
      ...
      if ( ip == np ) then
        ...
        Y => El%Sigma(:)
        if ( no /= sN_h ) call die('must be enforced')
      else
...
!$OMP end parallel do
  ...
  subroutine prep_HS(Z, El, spH, spS, &
    ...
    use class_zSpData1D
    use class_zSpData3D
    use m_tbt_tri_scat_spinor, only : insert_Self_Energy
    ...
    call c_f_pointer(c_loc(M), M2D, [2, n1, 2, n2])
    ...
    do iu = 1 , n2
      ...
      if ( l_ncol(io) /= 0 ) then
        ...
        do ju = 1 , n1
          ...
          if ( ind > 0 ) then
            ind = l_ptr(io) + ind
            M2D(1,ju,1,iu) = Z * S(ind) - H(1,1,ind)
            M2D(1,ju,2,iu) =            - H(1,2,ind)
            M2D(2,ju,1,iu) =            - H(2,1,ind)
            M2D(2,ju,2,iu) = Z * S(ind) - H(2,2,ind)
          end if
...
!$OMP end parallel
  ...
  pure function spin_projector(vec, anti_parallel)
    real(dp), intent(in), dimension(3) :: vec
    logical, intent(in), optional :: anti_parallel
    complex(dp), dimension(2,2) :: spin_projector 

    ! Convert polarization axis into projector matrix
    !       [ 1 + n_z     , n_x - i n_y]
    ! -0.5 *[ n_x - i n_y , 1 - n_z    ]
    !
    ! vec should already be normalized to 0.5
    !
    spin_projector(1,1) = cmplx(0.5_dp + vec(3), 0._dp, dp)
    spin_projector(1,2) = cmplx(vec(1),-vec(2), dp)
    spin_projector(2,1) = cmplx(vec(1), vec(2), dp)
    spin_projector(2,2) = cmplx(0.5_dp - vec(3), 0._dp, dp)
    if ( present(anti_parallel) ) then
      if (anti_parallel) then
        spin_projector(:,:) = I2 - spin_projector(:,:)
      end if
    end if
  end function spin_projector

  ! Project Gamma onto a give spin subspace
  subroutine P_Gamma_P(Gamma, no, proj1, proj2, PGammaP)
...
! * INPUT variables   *
! *********************
    integer, intent(in) :: no ! The dimension of Gamma and PGammaP
    complex(dp), intent(in), target :: Gamma(:), proj1(:,:), proj2(:,:)
    complex(dp), intent(inout), target :: PGammaP(:)

    ! local variables
    complex(dp), pointer :: PGammaP4D(:,:,:,:), Gamma4D(:,:,:,:)
    integer :: i, j, k, l

    call c_f_pointer(c_loc(Gamma), Gamma4D, [2, no, 2, no])
    call c_f_pointer(c_loc(PGammaP), PGammaP4D, [2, no, 2, no])
    ...
    do j = 1, 2
      do l = 1, 2
        do k = 1, 2
          do i = 1, 2
            PGammaP4D(i,:,j,:) = PGammaP4D(i,:,j,:) + proj1(i,k) &
                * Gamma4D(k,:,l,:) * proj2(l,j)
          end do
        end do
      end do
    end do

  end subroutine P_Gamma_P

  subroutine calc_sqrt_Gamma(no, Gamma, sqrt_Gamma, zwork)
...
! *********************
    integer, intent(in) :: no ! The dimension of Gamma and PGammaP
    complex(dp), intent(inout), target :: Gamma(:)
    complex(dp), intent(inout), target :: sqrt_Gamma(:), zwork(:)

    ! local variables
    complex(dp), parameter :: z0 = cmplx(0._dp, 0._dp, dp)
    complex(dp), parameter :: z1 = cmplx(1._dp, 0._dp, dp)
    complex(dp) :: evecs(4*no*no), ztmp(2*no), ztmp2(2*no)
    real(dp) :: evals(2*no)
    complex(dp), allocatable :: work(:)
    real(dp), allocatable :: rwork(:)
    integer, allocatable :: iwork(:)
    integer :: lwork, lrwork, liwork, ierr
    integer :: i, j, k, no2
    complex(dp) :: helper(4*no*no)

    complex(dp), external :: zdotu

    no2 = 2*no

    if ( no2*no2 > size(zwork) ) then
      write(*,'(a,tr1,i0)') 'sqrt_Gamma: Error in work array size.'
      call die('')
    end if

    call zcopy(no2*no2, Gamma(1), 1, zwork(1), 1)
    ...
    lrwork = -1
    liwork = -1
    call zheevd('V', 'L', no2, zwork(1), no2, evals, &
        work(1), lwork, rwork(1), lrwork, iwork(1), liwork,  &
        ierr)

    if ( ierr /= 0 ) then
      write(*,'(a,tr1,i0)') 'sqrt_Gamma: Error in worksize query:', ierr
      call die('')
    end if
    ...
    ! ---------------
    call zheevd('V', 'L', no2, zwork(1), no2, evals, &
        work(1), lwork, rwork(1), lrwork, iwork(1), liwork, ierr)

    if ( ierr /= 0 ) then
      write(*,'(a,tr1,i0)') 'sqrt_Gamma: Error in diagonalization:', ierr
      call die('')
    end if

    ! Calculate square root of Gamma
    !   sqrt(Gamma) = U sqrt(d) dagger(U)
    ! This could probably be done more efficiently
    call zcopy(no2*no2, zwork(1), 1, helper(1), 1)
    do k = 1, no2
      call zscal(no2, sqrt(cmplx(evals(k),0._dp,dp)), zwork((k-1)*no2+1), 1)
    end do
    
    call GEMM('N','C',no2,no2,no2,z1,zwork,no2,helper,no2,z0,sqrt_Gamma,no2)
    
    ! do k = 1, no2
    !   do i = 1, no2
    !     print*,i,k,Gamma((k-1)*no2+i), sqrt_Gamma((k-1)*no2+i), &
    !         (abs(Gamma((k-1)*no2+i)-sqrt_Gamma((k-1)*no2+i)))
    !   end do
    ! end do

    ! call zcopy(no2*no2, sqrt_Gamma(1), 1, helper(1), 1)
    ! call GEMM('N', 'N', no2, no2, no2, z1, sqrt_Gamma(1), no2, helper, no2, &
    !     z0, zwork, no2)

    ! do k = 1, no2
    !   do i = 1, no2
    !     print*,i,k,Gamma((k-1)*no2+i), zwork((k-1)*no2+i), &
    !         (abs(Gamma((k-1)*no2+i)-zwork((k-1)*no2+i)))
    !   end do
    ! end do
  ...
  end subroutine calc_sqrt_Gamma

  subroutine project_Gamma(no, sqrt_Gamma, proj, GammaP, zwork)
...
! *********************
    integer, intent(in) :: no ! The dimension of Gamma and PGammaP
    complex(dp), intent(in), target :: sqrt_Gamma(:), proj(:,:)
    complex(dp), intent(inout), target :: GammaP(:), zwork(:)

    ! local variables
    complex(dp), parameter :: z0 = cmplx(0._dp, 0._dp, dp)
    complex(dp), parameter :: z1 = cmplx(1._dp, 0._dp, dp)
    complex(dp), pointer :: zwork4D(:,:,:,:), sqrtG4D(:,:,:,:)
    integer :: i, j, k, l, no2

    no2 = 2 * no

    if ( no2*no2 > size(zwork) ) then
      write(*,'(a,tr1,i0)') 'project_Gamma: Error in work array size.'
      call die('')
    end if

    call c_f_pointer(c_loc(sqrt_Gamma), sqrtG4D, [2, no, 2, no])
    call c_f_pointer(c_loc(zwork), zwork4D, [2, no, 2, no])

    zwork(:) = 0._dp
    do j = 1, 2
      do i = 1, 2
        do k = 1, 2
          zwork4D(i,:,j,:) = zwork4D(i,:,j,:) + proj(i,k) * sqrtG4D(k,:,j,:)
        end do
      end do
    end do

    call zgemm('N', 'N', no2, no2, no2, z1, sqrt_Gamma(1), no2, zwork, no2, &
        z0, GammaP, no2)

  end subroutine project_Gamma

#undef GEMM

end module m_tbt_trik_spinor
