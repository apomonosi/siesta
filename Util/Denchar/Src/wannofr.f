...
! ---
      ...
      SUBROUTINE WANNOFR( NA, NO, no_u, MAXNA, NSPIN, nspin_blocks,
     .                   non_coll,
...
C **********************************************************************
C Compute the Wannier functions at the points of a plane or a 3D grid
C in real space
C     Coded by P. Ordejon, from Junquera's rhoofr. July 2003
!     Extended to NC spin by Alberto Garcia, 2019      
!     Extended to Wannier functions by Javier Junquera, 2019  
! Modified by A. Alcaraz to print out cube files with atoms, September 2024    
C **********************************************************************
...
! Stream over wavefunctions in file
      ...
      if (non_coll) then
        allocate(wf_single(4,1:no))
        allocate(wf(1:no,2))
        spinor_comps = 2
      else
        spinor_comps = 1
        if (gamma_wfsx) then
           allocate(wf_single(1,1:no))
           allocate(wf(1:no,1))
        else
           allocate(wf_single(2,1:no))
           allocate(wf(1:no,1))
        endif
      endif
...
!      write(6,*)' no           = ', no
!      write(6,*)' no_u         = ', no_u
!      write(6,*)' non_coll     = ', non_coll
!      write(6,*)' spinor_comps = ', spinor_comps
!      write(6,*)' gamma_wfsx   = ', gamma_wfsx
!      write(6,*)' nk           = ', nk
!      write(6,*)' nspin_blocks = ', nspin_blocks
!!     End debugging
     ...
     $              ' nwf: ', number_of_wfns
         ...
         wf_loop: DO IWF = 1, number_of_wfns
            ...
            read(wf_unit) (wf_single(:,io), io=1,no)
...
C Loop over Non-zero orbitals ------------------------------------------ 
     ...
     .               K(3)*(XPO(3)+XIJ(3,IAT1))
             ...
             DO IO = LASTO(IAVEC1-1) + 1, LASTO(IAVEC1)
               ...
               !     The Wannier functions do have components on 
               !     all the atomic orbitals of the supercell
               !     The difference with respect the wave functions
               !     is that the latter do have coefficients 
               !     only on the atomic orbitals of the unit cell
               CWAVE(:) = CWAVE(:) + PHIMU * WF(io,:) * EXPPHI 
            ENDDO       ! orbitals
         ENDDO ! atoms
