...
#      --out siesta-4.1-b3
#
# This script works by executing all codes in the top
# level directory.
# Hence it first figures out the top-level directory of
# the git repo, then cd's to it, and starts processing.
...
_year=$(date -d "$(git log -n1 --format='%ci' $_tag)" +"%Y")
...
sed -s -i -e "s/\\\\date{.*}/\\\\date{$_date}/" tbtrans.tex
sed -s -i -e "s/\\\\today/$_date/" siesta.tex
sed -s -i -e "s/\\\\the\\\\year{}/$_year/" siesta.tex
