...
!
!  It can be used to exchange matrices between Siesta and the
!  PEXSI library. Siesta uses a "block-cyclic" distribution over
!  N_Siesta processors, and PEXSI uses a simple "greedy" blocked
!  distribution on each pole-group, with npPerPole processors.

!  It can also be used to redistribute sparse matrices in ELSI when
!  sub-communicators (for spin, or k-points) are used.

!  The code works in principle with any distribution (subject to some
...
CONTAINS
 ...
 subroutine redistribute_spmatrix(norbs,m1,dist1,m2,dist2,bridge_comm)
   ...
   use mpi_siesta     ! Note that this is fine, as we do not use any Siesta-specific
   ...
   MPI_COMM_TYPE, intent(in)   :: bridge_comm    ! Umbrella Communicator
   ...
   integer ::  i, io, j, nvals, nvals_complex
   MPI_GROUP_TYPE :: g1, g2, gg
   integer ::  comparison, n1, n2
   MPI_COMM_TYPE :: c1, c2
...
!   if (myid == 0) print *, "Done transfers."
 ...
 !--------------------------------------------------
    subroutine do_transfers_int(comms,data1,data2,g1,g2,bridge_comm)
      ...
      MPI_GROUP_TYPE, intent(in)      :: g1
      MPI_GROUP_TYPE, intent(in)      :: g2
      MPI_COMM_TYPE, intent(in)       :: bridge_comm
 
      integer                 :: nsize1, nsize2, ierr
      MPI_GROUP_TYPE          :: basegroup
      integer, allocatable    :: comm_rank1(:), comm_rank2(:)
 
      integer :: ncomms
      integer :: i
      integer :: nrecvs_local, nsends_local
      MPI_STATUS_TYPE_2D :: statuses
      MPI_REQUEST_TYPE, allocatable :: local_reqR(:), local_reqS(:)
 ...
 !      print "(i4,a,2i3)", myrank,": g1 and g2: ", g1, g2
      ...
      ncomms = size(comms)
       ...
       ALLOC_MPI_STATUS_2D(statuses,nrecvs_local)
