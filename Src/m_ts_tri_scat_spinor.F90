...
! **********************
    ...
    integer :: i_Elec, idx_Elec, no, no2, nb
#ifdef TBTRANS
    integer :: i_diag
#endif
    integer :: np
    ...
    no = El%o_inD%n
    ! In non-collinear spin case each orbital is expanded
    ! a 2x2 block in GF
    no2 = 2 * no
    ...
    ! Convert indices of first and last electrode orbital
    ! in the device region to indices of block containing 
    ! them.
    sPart = which_part(A_tri, 2 * sPart - 1)
    ePart = which_part(A_tri, 2 * ePart - 1)
    ...
    i = 4 * 1024._dp**2 / 16._dp / no2
    i = max(2,i)
    nrtmp = size(ztmp) / no2
    if ( nrtmp < i ) then
      nrtmp = i
      nullify(ztmp)
      allocate(ztmp(no2*nrtmp))
      tmp_allocated = .true.
    else
      tmp_allocated = .false.
    end if
    ...
    do while ( i_Elec <= no )
      ...
      ! memory.
      if ( 2 * idx_Elec <= crows(sPart) ) then
        n = sPart
      else
      ...
      i = crows(n) - sN + 1
      sIdxF = (2*idx_Elec-1-i) * sN + 1
      eIdxF = (2*idx_Elec+nb-i) * sN
    ...
    do while ( i_Elec <= sNc )
      ...
      ! do Gf.Gamma
      call GEMM ('N','T',nb,no2,no2, z1, zwork(i_Elec), sNc, &
          El%Gamma(1), no2, z0, ztmp(1), nb)
      ...
      if ( present(TrGfG) ) then
        ...
        do i = 0 , nb - 1
          ...
          if ( in_rgn(El%inDpvt,(idx_Elec+i+1)/2) ) then
      ...
      do in = sPart , ePart
        ...
        call GEMM ('N','C',nb,sNo,no2, z1, ztmp(1), nb, &
  ...
  subroutine insert_Self_Energies(Gfinv_tri, Gfinv, pvt, El)
    ...
    integer :: no, off, i, j, io, jo, iu, ju, ii, jj, idx
    
    no = El%device_orbitals()
    off = El%idx_o - 1

    if ( El%Bulk ) then
    ! We iterate over the electrode orbitals (io,jo) [1..no] and over the 
    ! spin box (ii,jj) [1(up)..2(down)]. 
    ! El%Sigma is 'flattened', thus we convert the row indices in to an offset
    ! row_off = 2 * no * (row_idx-1)
    ! row_idx = 2 * (jo-1) + jj 
    ! 
!$OMP do private(jo,ju,j,jj,io,iu,i,ii,idx)
      do jo = 1 , no
        ! Index corresponding to electrode orbital jo
        ju = 2 * (pvt%r(jo+off)-1)
        do jj = 1, 2
          ! Offset of row in El%Sigma
          j = 2 * no * (2*(jo-1) + (jj-1))

          do io = 1 , no
            ! Index corresponding to electrode orbital io
            iu = 2 * (pvt%r(io+off)-1)
            do ii = 1, 2
              idx = index(GFinv_tri,iu+ii,ju+jj)
              Gfinv(idx) = El%Sigma(j+2*(io-1)+ii)
            end do
          end do
        end do
      end do
      ...
      do jo = 1 , no
        ...
        do jj = 1, 2
          ...
          do io = 1 , no
            ...
            do ii = 1, 2
              idx = index(GFinv_tri,iu+ii,ju+jj)
              Gfinv(idx) = Gfinv(idx) - El%Sigma(j+2*(io-1)+ii)
            end do
          end do
        end do
      end do
...
end module m_ts_tri_scat_spinor
