...
module m_elsi_interface
  ...
  integer, parameter :: BUILD_DM_UNSET               = 0
  integer, parameter :: BUILD_DM_RANK_UPDATE         = 1
  integer, parameter :: BUILD_DM_ELPA_MULTIPLY_AT_A  = 2
  integer, parameter :: BUILD_DM_ELPA_MULTIPLY       = 3
  integer, parameter :: BUILD_DM_PDGEMM              = 4
  integer, parameter :: BUILD_DM_ELPA_PXGEMM         = 5
  ...
  MPI_COMM_TYPE, save :: elsi_global_comm    ! Used by all routines. Freed at end of scf loop
  MPI_COMM_TYPE, save :: kpt_comm ! Freed at end of scf loop
  MPI_COMM_TYPE, save :: elsi_Spatial_comm ! Freed at end of scf loop
  MPI_COMM_TYPE, save :: elsi_spin_comm ! Freed at end of scf loop
  ...
  character(len=18) :: build_dm_string
...
subroutine elsi_get_opts()
  ...
  build_dm_string      = fdf_get("ELSI-Build-DM-Method", "none-preferred")
  ...
  case ("elpa", "ELPA")
    ...
    if (IONode) then
       ...
       call add_citation("10.1016/j.parco.2011.05.002")
       if (elpa_gpu==1) then
  ...
  case ("pexsi", "PEXSI")
    call elsi_get_pexsi_enabled(enabled)
    if (enabled == 1) then     
       if (IONode) then
       ...
       call die("The ELSI library does not have support for the PEXSI solver")
    endif
  case ("sips", "SIPS", "SIPs")
    call elsi_get_sips_enabled(enabled)
    if (enabled == 1) then     
       which_solver = SIPS_SOLVER
    else
       call die("The ELSI library does not have support for the SIPS solver")
    endif
  case ("ntpoly", "NTPOLY", "NTPoly")
    which_solver = NTPOLY_SOLVER
    if (IONode) then
       call add_citation("10.1016/j.cpc.2017.12.010")
    endif
  case ("eigenexa", "EigenExa", "EIGENEXA")
    call elsi_get_eigenexa_enabled(enabled)
    if (enabled == 1) then     
       which_solver = EIGENEXA_SOLVER
    else
       call die("The ELSI library does not have support for the EigenEXA solver")
    endif
  case ("MAGMA", "magma")
    call elsi_get_magma_enabled(enabled)
    if (enabled == 1) then     
       which_solver = MAGMA_SOLVER
    else
       call die("The ELSI library does not have support for the MAGMA solver")
    endif
  case default
    which_solver = ELPA_SOLVER
  end select
  ...
  select case (build_dm_string)
  case ("pdgemm")
     which_build_dm_method = BUILD_DM_PDGEMM
  case ("rank-update")
     which_build_dm_method = BUILD_DM_RANK_UPDATE
  case ("elpa-multiply-at-a")
     which_build_dm_method = BUILD_DM_ELPA_MULTIPLY_AT_A
  case ("elpa-multiply")
     which_build_dm_method = BUILD_DM_ELPA_MULTIPLY
  case ("elpa-pxgemm")
     which_build_dm_method = BUILD_DM_ELPA_PXGEMM
  case default
     which_build_dm_method = BUILD_DM_UNSET
  end select
...
#ifdef MPI
  use mpi_siesta, only : MPI_Comm_Dup, MPI_Comm_DFT, MPI_integer
  use mpi_siesta, only : MPI_AllReduce,MPI_sum, MPI_Comm_Rank
  use mpi_siesta, only : MPI_Comm_Free, MPI_Double_Precision, MPI_AllGatherV
#endif
  use class_Distribution
...
! --- MAGMA
    ...
    call elsi_set_build_dm_method(elsi_h, which_build_dm_method)
 ...
 if ( (which_solver == PEXSI_SOLVER) .and. &
    ...
    if (n_spin == 1) then
       ...
       call elsi_set_mpi(elsi_h, MPI_COMM_ID(elsi_global_comm))
       ...
       call elsi_set_mpi(elsi_h, MPI_COMM_ID(elsi_Spatial_comm))
       call elsi_set_mpi_global(elsi_h, MPI_COMM_ID(elsi_global_comm))
...
subroutine elsi_kpoints_dispatcher(iscf, no_s, nspin, no_l, maxnh, no_u,  &
  ...
  use mpi_siesta, only: mpi_comm_dft, mpi_comm_dup, mpi_comm_rank
  use mpi_siesta, only: mpi_comm_size, mpi_comm_split, mpi_comm_group
  use mpi_siesta, only: mpi_group_translate_ranks, mpi_group_free
  use mpi_siesta, only: MPI_DOUBLE_PRECISION, MPI_INTEGER, MPI_SUM
  use mpi_siesta, only: mpi_allgather, mpi_barrier, mpi_reduce
  use mpi_siesta, only: MPI_Comm_Free
  use parallel, only: blocksize
  use class_Distribution
    use m_redist_spmatrix, only: aux_matrix, redistribute_spmatrix
    use alloc, only: de_alloc  ! To deallocate some pointers in transfer matrices
  ...
  use m_fold_auxcell, only: fold_sparse_arrays ! Could be called in state_init
     ...
     integer, intent(in), target ::  listh(maxnh), numh(no_l)
      ...
      MPI_COMM_TYPE :: kpt_col_comm
      MPI_GROUP_TYPE :: Global_Group, kpt_Group
...
subroutine elsi_save_potential(n_pts, n_spin, v_scf, comm)
  ...
  MPI_COMM_TYPE , intent(in) :: comm  ! The Siesta communicator used for grid operations
...
subroutine get_spin_comms_and_dists(global_comm, base_comm, blocksize, n_spin,&
  ...
  MPI_COMM_TYPE, intent(in) :: global_comm    ! Communicator for global distribution
                                         ! Needed to include the actual global ranks
                                         ! in the distribution objects

  MPI_COMM_TYPE, intent(in)  :: base_comm      ! Communicator to be split
  MPI_COMM_TYPE, intent(out) :: spatial_comm   ! "Orbital" comm (one for each spin team)
  MPI_COMM_TYPE, intent(out) :: spin_comm      ! "Inner" spin comm (for reductions)
  ...
  MPI_GROUP_TYPE :: global_group, Spatial_group
...
subroutine elsi_complex_solver(iscf, n_basis, n_basis_l, n_spin, nnz_l, numh, row_ptr, &
  ...
  MPI_COMM_TYPE, intent(in)    :: kpt_comm
  integer,  intent(in)    :: neigwanted
...
#ifndef SIESTA__ELSI_DOES_NOT_HAVE_PEXSI_METHOD
    ! This routine was introduced in 2.4.1
    call elsi_set_pexsi_method(elsi_h, pexsi_method)
#endif
...
! --- MAGMA
 ...
 if ( (which_solver == PEXSI_SOLVER) .and. &
    ...
    if (n_spin == 1) then
       ...
       call elsi_set_mpi(elsi_h, MPI_COMM_ID(kpt_comm))
       call elsi_set_mpi_global(elsi_h, MPI_COMM_ID(elsi_global_comm))
