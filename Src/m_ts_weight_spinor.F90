...
contains
  ...
  subroutine weight_DM_spinor(N_Elec,Elecs,N_mu, mus, na_u, lasto, &
    ...
    use m_ts_weight, only : TS_W_ORB_ORB, TS_W_TR_ATOM_ATOM, TS_W_SUM_ATOM_ATOM, &
                            TS_W_TR_ATOM_ORB, TS_W_SUM_ATOM_ORB, TS_W_MEAN, &
                            TS_W_CORRELATED, TS_W_METHOD
    ...
    type(dSpData3D), intent(inout) :: spDMneq
    ! Estimates of EDM
    ...
    ! * LOCAL variables   *
    ! *********************
    real(dp) :: w(N_mu), neq(spin%DM,N_mu)
            ...
            if ( .not. is_Trace ) then ! TS_W == TS_W_SUM_ATOM_?
              ...
              if ( sum(abs(sc_off(:,is))) > EPS ) cycle lio_connect
                ...
                if ( is_correlated ) then
                  atom_neq(:,ia) = atom_neq(:,ia) + (DMneq(1,ind,:) + DMneq(2,ind,:))
                else
                  atom_neq(:,ia) = atom_neq(:,ia) + (DMneq(1,ind,:) + DMneq(2,ind,:))**2
            end if
      ...
      if ( l_ncol(lio) /= 0 ) then
        ...
        do sj = 1, s_ncol(lio)
          ...
          if ( TS_W == TS_W_ORB_ORB ) then
            ...
            call calc_neq_weight(N_mu,N_nEq_ID,spin%DM,ID_mu, &
                cor,neq,w)
            ...
            call calc_neq(N_mu,N_nEq_ID,spin%DM,ID_mu,cor,neq)
          ...
          if ( io == ucorb(jo,ng) .and. io == 28 ) then
            print '(2(a7,4(tr1,f10.5)))','Left',DM(1:2,ind,1),neq(1),w(1), &
                'Right',DM(1:2,ind,2),neq(2),w(2)
          end if
#endif
          ...
          if ( hasEDM ) then
            ...
            do mu_i = 1 , N_mu - 1
              do mu_j = mu_i + 1 , N_mu
                ! Density matrix
                tmp = DM(1,ind,mu_i) - DM(1,ind,mu_j) & 
                    + DM(2,ind,mu_i) - DM(2,ind,mu_j) 
                ! Calculate sum of all errors
                m_err = m_err + tmp
                if ( abs(tmp) > abs(ee) ) ee = tmp

                ! Energy density matrix
                tmp = EDM(1,ind,mu_i) - EDM(1,ind,mu_j) & 
                    + EDM(2,ind,mu_i) - EDM(2,ind,mu_j) 
                Em_err = Em_err + tmp
            ...
            e_f  = DM(1,ind,1) + DM(2,ind,1)
            Ee_f = EDM(1,ind,1) + EDM(2,ind,2)

            ! Also calculate the charge after weighting
            q(1) = q(1) + w(1) * (DM(1,ind,1) + DM(2,ind,1)) * S(sind)
            DM(:,ind,1) = w(1) * DM(:,ind,1)
            EDM(:,ind,1) = w(1) * EDM(:,ind,1)
            do mu_i = 2 , N_mu
              q(mu_i) = q(mu_i) + w(mu_i) * (DM(1,ind,mu_i) + DM(2,ind,mu_i)) * S(sind)
              DM(:,ind,1) = DM(:,ind,1) + w(mu_i) * DM(:,ind,mu_i)
              EDM(:,ind,1) = EDM(:,ind,1) + w(mu_i) * EDM(:,ind,mu_i)
            end do

            ! Calculate error from estimated density
            e_f = e_f - DM(1,ind,1) - DM(2,ind,1)
            Ee_f = Ee_f - EDM(1,ind,1) - EDM(2,ind,1)
            do mu_i = 2 , N_mu
              tmp = DM(1,ind,mu_i) - DM(1,ind,1) &
                  + DM(2,ind,mu_i) - DM(2,ind,1)
              if ( abs(tmp) > abs(e_f) ) e_f = tmp
              tmp = EDM(1,ind,mu_i) - EDM(1,ind,1) &
                  + EDM(2,ind,mu_i) - EDM(2,ind,1)
...
!$OMP end parallel do
    ...
    ! TODO why not use: N_mu * (N_mu - 1) / 2
    io = 0
    do mu_i = 1, N_mu - 1
      do mu_j = mu_i + 1 , N_mu
        io = io + 1
    ...
    if ( Nodes > 1 ) then
      ...
      allocate(DM(2,Nodes,1))
      w(1) = eM
      w(2) = EeM

      ! First figure out which process it is
      call MPI_Gather(w(1),2,MPI_Double_Precision, &
          DM(1,1,1),2,MPI_Double_Precision, &
          0, MPI_Comm_World, MPIerror)
      if ( Node == 0 ) then
        ! Figure out which node contains the largest error
        ia1 = 0
        neq(1,1) = DM(1,1,1) ! density matrix
        ia2 = 0
        neq(1,2) = DM(2,1,1) ! energy density matrix
        do io = 2 , Nodes
          if ( abs(DM(1,io,1)) > abs(neq(1,1)) ) then
            neq(1,1) = DM(1,io,1)
            ia1 = io - 1
          end if
          if ( abs(DM(2,io,1)) > abs(neq(1,2)) ) then
            neq(1,2) = DM(2,io,1)
            ia2 = io - 1
          end if
        end do
      ...
      allocate(DM(3,2,1))
      DM(1,1,1) = real(n_nzs,dp)
      DM(2,1,1) = m_err   ! DM mean-error
      DM(3,1,1) = Em_err  ! EDM mean-error
      call MPI_Reduce(DM(1,1,1),DM(1,2,1),3,MPI_Double_Precision, &
      ...
      ! Density matrix
      DM(1,1,1) = real(eM_i,dp)
      DM(2,1,1) = real(eM_j,dp)
      DM(3,1,1) = eM ! maximum difference between mu_i
      DM(4,1,1) = DMe ! final density at max-diff
      DM(5,1,1) = ew ! maximum difference from final density
      if ( Node == 0 .and. Node == ia1 ) then
        ! do nothing, everything is updated
      else if ( Node == 0 ) then
        ! recv from main node
        call MPI_Recv(DM(1,1,1),5, MPI_Double_Precision, &
        ...
        ! send to main node
        call MPI_Send(DM(1,1,1),5, MPI_Double_Precision, &
            0, 0, MPI_Comm_World, MPIerror)
      end if

      ! Energy density matrix
      DM(1,1,1) = real(EeM_i,dp)
      DM(2,1,1) = real(EeM_j,dp)
      DM(3,1,1) = EeM
      DM(4,1,1) = EDMe
      DM(5,1,1) = Eew
  ...
  end subroutine weight_DM_spinor
  ...
  pure subroutine calc_weight(N_mu,N_id,ID_mu,w_ID,w)
    ...
    call calc_neq(N_mu,N_id,1,ID_mu,w_ID,theta)
  ...
  pure subroutine calc_neq_weight(N_mu,N_id, nspin, ID_mu,neq_ID,neq,w)
    integer,  intent(in)  :: N_mu, N_id, nspin, ID_mu(N_id)
    real(dp), intent(in)  :: neq_ID(nspin,N_id)
    real(dp), intent(out) :: neq(nspin,N_mu)
    real(dp), intent(out) :: w(N_mu)
    integer :: i, j
    real(dp) :: tmp

    ! TODO check that this is correct for several electrodes
    call calc_neq(N_mu,N_id,nspin,ID_mu,neq_ID**2,neq)
    tmp = 0._dp
    do i = 1 , N_mu
    ...
    call calc_neq(N_mu,N_id,nspin,ID_mu,neq_ID,neq)
  ...
  pure subroutine calc_neq(N_mu,N_id,nspin,ID_mu,neq_ID,neq)
    integer,  intent(in)  :: N_mu, N_id, nspin, ID_mu(N_id)
    real(dp), intent(in)  :: neq_ID(nspin,N_id)
    real(dp), intent(out) :: neq(nspin,N_mu)
    integer :: ID
