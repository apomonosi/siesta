...
module partial_charges_m
  ...
  integer, public :: want_partial_charges = 0
...
! ----------------------------------------------------------------------
    ...
    use sys
    use siesta_options, only: option_charges
#ifdef MPI
    use mpi_siesta
#endif
...
! ----------------------------------------------------------------------
    if ( option_charges%hirshfeld%run(want_partial_charges) ) then
...
#ifdef MPI
      call MPI_Reduce(q(1,1), qtmp(1,1),nspin*na_u, &
...
! ----------------------------------------------------------------------
    if ( option_charges%voronoi%run(want_partial_charges) ) then
  ...
  contains
    ...
    subroutine alloc_q()
       ...
       call memory('A','D',na_u,'partial_charge')
#ifdef MPI
...
#ifdef MPI
       call memory('D','D',size(qtmp),'partial_charge')
       deallocate(qtmp)
#endif
       call memory('D','D',size(q),'partial_charge')
       deallocate(q)
  ...
  subroutine write_partial_charges(nspin, na_u, q, method)
    ...
    use atmfuncs, only: labelfis, zvalfis
    use siesta_cml
    ...
    !< Which origin of the charges is it?
    !!
    !! 1 == Hirshfeld
    !! 2 == Voronoi
    !! 3 == Mulliken
    ...
    integer :: ia, is
    real(dp) :: dqtot, S, Stot, Svec(3)
    real(dp), allocatable :: qout(:,:)

    ! quick-return if not io-node
    ...
    if ( method == 1 ) then
      ...
      write(6,"(/,a)") 'Mulliken Atomic Populations:'
      if ( cml_p ) then
        call cmlStartPropertyList(xf=mainXML, dictRef='siesta:mulliken', &
            title='Mulliken populations')
      end if
    else
      call die("Unknown method for write_partial_charges")
    end if
    ...
    ! number of electrons.
    ! The sum of them == charge-neutral!
    write(6,'(a6,1x,2(tr1,a11))', advance='no') &
       'Atom #', 'charge [q]', 'valence [e]'

    select case ( nspin )
    case ( 1 )
      write(6,'(tr2,a)') 'Species'
      fmt = "(i6,1x,2(tr1,f11.6),2x,a)"
    ...
    case ( 2 )
      write(6,'(tr1,a11,2x,a)') spin_label // 'z [e]', 'Species'
      fmt = "(i6,1x,3(tr1,f11.6),2x,a)"
    ...
    case ( 4 )
      write(6,'(4(tr1,a11),2x,a)') & 
          spin_label, spin_label//'x [e]', spin_label//'y [e]', spin_label//'z [e]', 'Species'
      fmt = "(i6,1x,6(tr1,f11.6),2x,a)"
    ...
    if ( cml_p ) then
      call cmlAddProperty(xf=mainXML, title="Atomic net charge", &
          ...
          units="siestaUnits:|e|")
      call cmlAddProperty(xf=mainXML, title="Atomic valence population", &
          dictRef='siesta:atom_valence_population', value=qout(2,:), &
          units="siestaUnits:e")
    end if
    ...
    select case ( nspin )
    case ( 1 )
      write(6,'(a)') repeat('-',6+2+12)
      write(6,'(a6,2x,f11.6)') 'Total', dqtot
    case ( 2 )
      write(6,'(a)') repeat('-',6+1+3*12)
      write(6,'(a6,2x,f11.6,12x,tr1,f11.6)') 'Total', dqtot, Stot
    case ( 4 )
      write(6,'(a)') repeat('-',6+1+6*12)
      write(6,'(a6,2x,f11.6,12x,4(tr1,f11.6))') 'Total', dqtot, Stot, Svec
    end select
